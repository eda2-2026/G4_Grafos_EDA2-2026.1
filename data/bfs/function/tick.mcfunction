...
execute as @e[type=block_display,tag=bfs_block] at @s if entity @e[type=marker,tag=bfs_target,distance=..0.3] run scoreboard players add global bfs_target_index 1
execute as @e[type=block_display,tag=bfs_block] at @s if entity @e[type=marker,tag=bfs_target,distance=..0.3] run playsound minecraft:entity.chicken.step master @a ~ ~ ~ 1 1

# 5. Checa se ele chegou no fim do labirinto (O target index ficou maior que a distância total)
execute as @e[type=block_display,tag=bfs_block] at @s if score global bfs_target_index > global bfs_dist run playsound minecraft:entity.player.levelup master @a ~ ~ ~ 1 1
execute as @e[type=block_display,tag=bfs_block] at @s if score global bfs_target_index > global bfs_dist run kill @s
...
execute if score global cfg_numbers matches 1 as @e[type=block_display,tag=bfs_block] at @s if entity @e[type=marker,tag=bfs_target,distance=..0.3] run function bfs:display/color_text_green with storage bfs:macro
